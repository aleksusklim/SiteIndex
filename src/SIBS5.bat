...
rem . SIBS (Site Index Batch Script) v1.5, Kly_Men_COmpany!
...
set ignore=.tmp
set include=include.tmp
set noindex=noindex.tmp
set copyto=.\_index_.tmp\
...
if not exist "%way%" mkdir "%way%"
copy /y %index% "%way%" >nul
del /q /f %tmpr%
for /d %%i in (*) do call :tree "%%~i"
if "%~1" == "" copy /y %sibs%.txt "%copyto%%sibs%.txt"
goto :eof
:dirs
